...
const InvoicesPageProject = (props) => {
  ...
  projectAreas={invoices[0].invoiceItems.map(item => item.invoiceItemArea)}
  projectDepths={invoices[0].invoiceItems.map(item => item.invoiceItemDepth)}
  projectWidths={invoices[0].invoiceItems.map(item => item.invoiceItemWidth)}
  projectLengths={invoices[0].invoiceItems.map(item => item.invoiceItemLength)}
  projectUnits={invoices[0].invoiceItems.map(item => item.invoiceItemMeasurement)}
  projectPrices={invoices[0].invoiceItems.map(item => item.invoiceItemPrice)}
  projectTotal={invoices[0].invoiceTotalPrice}
  projectEdges={invoices[0].invoiceItems.map(item => item.edgeProfile.edgeProfileCut)}
  projectEdgeMeasures={invoices[0].invoiceItems.map(item => item.edgeProfile.edgeProfileMeasurement)}
  projectEdgeTypes={invoices[0].invoiceItems.map(item => item.edgeProfile.edgeProfileType)}
/>
